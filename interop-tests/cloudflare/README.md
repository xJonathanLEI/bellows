...
Scheduled recovery scenarios run in all four language topologies, covering lost dispatches and scheduling hints, eligibility and lease races, overlapping sweeps, paged fan-out, invalid candidates, schema isolation, and recovery after failure. They verify reuse of existing IDs and observable scheduled-event failures in both languages.
...
Dispatcher durability starts with a persisted `retryAt` hint, not acceptance; Cron recovers earlier invocation loss. Attempts remain at-least-once, so use idempotent side effects.
