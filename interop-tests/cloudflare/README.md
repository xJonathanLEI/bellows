...
The [TypeScript harness](../../bellows-ts/test/integration/cloudflare/README.md) owns the five TypeScript -> TypeScript cases. The [Rust harness](../../bellows/tests/integration/cloudflare/README.md) owns five Rust -> Rust cases plus all 14 Rust workerd contracts. Root `pnpm test` runs the three packages serially and covers all **34 Cloudflare cases exactly once**. The existing native interop tests and their intentional TypeScript build in `interop-tests/build.rs` are separate and unchanged.
