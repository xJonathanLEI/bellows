...
Pending and in-flight reconstruction replaces the delegate while retaining real Durable Object storage; it does not induce platform eviction. The topology suite uses real automatic alarms, without manual alarm calls or a second dispatch. Deterministic native/TypeScript tests cover the long watchdog, stale results, storage failures, and 300-ID unrestricted fan-out without waiting sixty seconds. All fixture-only inspection, reconstruction, response-loss, and cleanup routes must be removed before deployment.
...
The publisher owns Bellows resources and dispatch orchestration, not an HTTP endpoint or arbitrary business clients. Your handler validates input and awaits `publisher.publish(&env, payload)` or `publisher.publish_future(&env, payload, available_from)`, using `bellows::time::Instant`. Future publication still dispatches immediately; the processor obtains PostgreSQL's availability hint for the DO's later alarm. Configuration callbacks and publication futures have no extra `Send` requirement. Delegates may be reused without retaining connections; callback-bearing tasks publish without callback registration, and singleton tasks are rejected. Awaitable publication and application cleanup hooks are not added.

The typed result is `Result<PostgresPublisherReceipt, PostgresPublisherError>`, not `worker::Result`. Success retains `task_id: String` and confirms dispatch acceptance, not completion. PostgreSQL processors require canonical positive decimal IDs no greater than `9007199254740991`; the lower-level Rust backend still returns exact `u64` receipts above this range. TypeScript instead throws `PostgresPublishedTaskIdError` for unsafe lower-level numeric receipts, retaining the exact ID as a string.
...
The event wrapper in [`processor/lib.rs`](./processor/lib.rs) shares request-local cleanup across its factories. See the [registration example](../../../../README.md#processor-and-deployment) for configuration.
...
Direct `PostgresPublishingBackend` plus `dispatch_task` also remains available for caller-managed publication. Import `TaskPublishingBackend`, connect inside the request, and await `close()` on success and error paths before dispatch or response; dropping clones is insufficient. See the [publishing example and capability limits](../../../../README.md#publishing-without-a-listener).
